	...
	$.ajax({
		...
		if(user != null) {
			if(user.position == 'NOOB') {
				...
				$('#sendAuthMailBtn').attr("onclick", "sendVerification('" + user.usrId + "', '" + user.usrName + "', '" + user.nickname + "')");
			} else {
				$('#authPlease').remove();
			...
			$('#usrClsIcon').attr("src", "/godgamez/res/user/icon/" + user.usrIcon + ".jpg");
			$('#nickname').text(user.nickname);
			$('#gold').text(user.gold);
			$('#usrGold').text(user.gold);
			$('#usrLv').text(parseInt(user.usrLv));
			exp = (user.usrLv - parseInt(user.usrLv)) * 100;
			$('#expPoint').attr("value", exp);
			$('#expPoint').attr("title", exp + "%");
	        
			getUsrclss(user.usrCode);
			getUsrqsts(user.usrCode);
			getCoupons(user.usrCode);
		}// else modal("", "", "실패6");
	})//.fail(() => {
		//modal("", "", "실패6");
	//})
...
function getAccptQsts(accptUsrqsts) {
	...
	if(accptPage - 1) {
		...
		$.each(accptUsrqsts, (idx, accpt) => {
			...
			let pushItem = `<td id='accptQstImg' onclick='location.href="/godgamez/quest/quest?qstId=\${accpt.qst.qstId}"'>
								<img src='/godgamez/res/quest/\${accpt.qst.qstId}.jpg' alt='수락한 퀘스트' name='accptQstImg' title='수행하러 가기'>
			...
			if(idx + 1 == accptTot) {
				...
				if(accptPage*3 - accptTot)
					for(i=0; i<accptPage*3 - accptTot; i++)
						accptUsrqstList.push(`<td id='accptQstImg'><img src='/godgamez/res/userQuest/none.jpg' name='accptQstImg'></td>`);
		...
		$.each(accptUsrqsts, (idx, accpt) => {
			...
			accptUsrqstList.push(
				`<td id='accptQstImg' onclick='location.href="/godgamez/quest/quest?qstId=\${accpt.qst.qstId}"'>
					<img src='/godgamez/res/quest/\${accpt.qst.qstId}.jpg' alt='수락한 퀘스트' name='accptQstImg' title='수행하러 가기'>
					<span id='accptQstName'>\${qstName}</span>
				</td>`
			);
	...
	if(donePage - 1) {
		...
		$.each(doneUsrqsts, (idx, done) => {
			let pushItem = `<td id='doneQstImg' onclick='getDoneDetail("\${done.modDate}", \${done.qst.qstId}, \${done.usr.usrCode}, "\${done.qst.qstName}")'>
								<img src='/godgamez/res/userQuest/\${done.usr.usrCode}_\${done.qst.qstId}.jpg' alt='퀘스트 제출 이미지' name='doneQstImg' title='크게 보기'>
			...
			if(idx + 1 == doneTot) {
				...
				if(donePage*3 - doneTot)
					for(i=0; i<donePage*3 - doneTot; i++)
						doneUsrqstList.push(`<td id='doneQstImg'><img src='/godgamez/res/userQuest/none.jpg' name='doneQstImg'></td>`);
...
function getDoneDetail(modDate, qstId, usrCode, qstName) {
	...
	$('#doneQstImgDetailModal #qstImgDetail').attr("src", "/godgamez/res/userQuest/" + usrCode + "_" + qstId + ".jpg");
...
function delDoneQstImg(qstId, usrCode) {
	$.ajax({
		url: "/godgamez/user/quest/del/img",
		type: "POST",
        contentType: "application/json",
		data: JSON.stringify({
			usrCode: usrCode,
...
function getCoupons(usrCode) {
	...
	$.ajax({
		url: '/godgamez/user/coupon/list/' + usrCode,
	}).done(cpns => {
		if(cpns.length) {
			$('#usrCpnItems').empty();
					...
					if(cpn.availability == 1) {
						...
						pushItem =
							`<td id='usrCpnImg' onclick='getCpnDetail("\${store}", "\${storeKor}", "\${dcPer}", \${cpn.cpnCode}, \${usrCode})'>
								...
								<img src='/godgamez/res/coupon/\${store}.jpg' class='btn' alt='쿠폰 사용처 이미지' name='usrCpnImg'>
							</td>`
					} else {
						pushItem =
							`<td id='usrCpnImg'>
								<div id='availability'><span>사용 완료</span></div>
								<img class='opacity-50' src='/godgamez/res/coupon/\${store}.jpg' alt='쿠폰 사용처 이미지' name='usrCpnImg'>
							</td>`
					}
					...
					if(idx + 1 == cpnTot) {
						...
						if(cpnPage*3 - cpnTot)
							for(i=0; i<cpnPage*3 - cpnTot; i++)
								cpnList.push(`<td id='usrCpnImg'><img src='/godgamez/res/userQuest/none.jpg' name='usrCpnImg'></td>`);
...
function getCpnDetail(store, storeKor, dcPer, cpnCode, usrCode) {
	...
	$('#cpnDetailModal #cpnImgDetail').attr("src", "/godgamez/res/coupon/" + store + ".jpg");
...
function useCpn(cpnCode, usrCode) {
	$.ajax({
		url: "/godgamez/coupon/get/" + cpnCode,
	}).done(cpn => {
		$.ajax({
			url: "/godgamez/coupon/useCoupon",
			type: "PUT",
	        contentType: "application/json",
...
<div id='body'>
			...
			<div class='row justify-content-center'>
				...
				<div class='col align-self-end'>
					<small class='float-right'>
						<a href='/godgamez/user/quit/step1' class='text-light'>회원탈퇴</a> &nbsp;|&nbsp;
						<a href='/godgamez/user/modify/step1' class='text-light font-weight-bold'>회원정보수정</a>
					</small>
				</div>
			</div>
					...
					<div class='row'>
						...
						<div class='col d-flex justify-content-end align-self-end mr-3'>
							<table id='usrQst' class='h-25 mr-3 text-primary'>
								<tr>
									<td rowspan='2'>
										<img src='/godgamez/res/user/mypage_quest_icon.png' alt='퀘스트 아이콘' id='usrQstIcon'
											class='imgBtn text-primary' onclick='location.href="/godgamez/user/mypage#questAlbum"'>
									</td>
							...
							<table id='usrCpnAndGold' class='h-25 mr-3 text-primary'>
								<tr>
									<td rowspan='2'>
										<img src='/godgamez/res/user/mypage_coupon_icon.png' alt='쿠폰 아이콘' id='usrCpnIcon'
											class='imgBtn' onclick='location.href="/godgamez/user/mypage#myCoupon"'>
									</td>
	...
	<div class='container usrDetail'>
		<div class='row border d-flex justify-content-between py-3' id='authPlease' hidden='true'>
			...
			<div class="col-5">
				<a class="btn btn-secondary float-right mt-2" href="/godgamez/user/verifiy">
		...
		<div class='row mt-5' oncontextmenu="return false" ondragstart="return false">
			...
			<div class='col mt-5 mr-5'>
				<div class='float-right text-secondary small btn' onclick='location.href="/godgamez/quest/board"' title='골드 벌러 가기 ☞'>
					<i class="fas fa-coins"></i>&nbsp;&nbsp;
					<span id='usrGold' class='h6'></span>
				</div>
				<div class='float-right text-secondary small btn' onclick='location.href="/godgamez/coupon/shop"' title='쿠폰 사러 가기 ☞'>
					<i class="fas fa-ticket-alt"></i>&nbsp;&nbsp;
