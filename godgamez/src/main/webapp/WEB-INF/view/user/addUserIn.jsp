...
function getUserCode() {
	$.ajax({
		url: "/godgamez/user/userCode/next",
		async: false
	}).done(usercode => {
		if(usercode.lenth) return usercode;
		else {
			modal("회원코드", "조회", "실패");
			return null;
		}
...
// 중복인지 아닌지, 비어있는 지 아닌 지, GM으로 시작하는 지
function nickChk(job) {
	...
	if(nick.length) {
		if(nick.length >= 2 && nick.search(/[가-힣0-9a-zA-Z]/) != -1 && 6 >= nick.length
...
function reset(userCode) {
	$.ajax({
		url: "/godgamez/user/del/" + userCode,
		type: "DELETE",
	})
	modal("회원", "가입", "실패");
}
...
function sendMails(userCode) {
	...
	welcomeData = {
		...
		text: "<a href='http://localhost/godgamez/'>"+
				"<div style='margin: 0;'>"+
	...
	<div class='container' onmousemove='phoneChk(1); nameChk(1); pwChk(1); pwAgainChk(1); idChk(1); nickChk(1); clsLimit(); chkInput(5);'>
		<div class='row'>
			<div class='col'>
				...
				<div class='row mx-5'>
					<div class='col mt-4'>
						<div class='row mb-4 usrIn'>
							<h6 class='font-wieght-bold' id='must'>*</h6>
							<input type='text' class='form-control' name='usrIn' id='usrId' value='nice920525@naver.com'
								placeholder=' ID로 사용할 이메일 주소를 입력하세요.' onchange='idChk(1); chkInput(5);' required>
							<h6 class='font-wieght-bold' id='usrIdChk'></h6>
						</div>
						<div class='row mb-2 usrIn'>
							<h6 class='font-wieght-bold' id='must'>*</h6>
							<input type='password' class='form-control' name='usrIn' id='usrPw' value='wjddn123!'
						...
						<div class='row mb-2 usrIn'>
							<h6 class='font-wieght-bold' id='must'>*</h6>
							<input type='password' class='form-control' name='usrIn' id='usrPwAgain' value='wjddn123!'
				...
				<div class='row mx-5'>
					<div class='col mt-4 mr-3'>
						<div class='row mb-2 usrIn'>
							<h6 class='font-wieght-bold' id='must'>*</h6>
							<input type='text' class='form-control' name='usrIn' id='usrName' value='송정우'
						...
						<div class='row mb-2 usrIn'>
							<h6 class='font-wieght-bold' id='must'>*</h6>
							<input type='text' class='form-control' onfocus='(this.type="date")' onblur='(this.type="text")' value='1992-05-25'
					...
					<div class='col mt-4 ml-3'>
						...
						<div class='row mb-2 usrIn'>
							<h6 class='font-wieght-bold' id='must'>*</h6>
							<input type='tel' class='form-control' name='usrIn' id='usrPhone' value='01055210689'
				...
				<div class='row mt-1 pl-2 justify-content-between'>
					<h4>＊CLASS</h4>
					<h6><a class='small font-wieght-bold text-dark font-italic mr-3' href='/godgamez/guide/aboutUs#classGuide' target='_blank'>
						클래스란? >
					</a></h6>
				</div>
						...
						<div class='row' id='usrClsSetting' onmousedown='clsLimit(); chkInput(5);'>
							<div class='col ml-0 pl-0 w-25'>
								<img class='border' id='clsIcon'
									 alt='공부 아이콘' name='STUDY' src='/godgamez/res/user/icon/STUDY.jpg'>
				...
				<div class='row mx-3 justify-content-around'>
					...
					<button class='btn btn-secondary col' id='nextStepBtn' title='필수사항 입력 후 다음 단계로 진행할 수 있습니다.' onclick='addUser()'>다음</button>
				</div>
			</div>
		</div>
	</div>
