...
function getConnectedUser() {
	$.ajax({
		...
		if(user != null) {
			userCode = user.usrCode;
			
			$('.usrIn #usrId').val(user.usrId);
			$('.usrIn #usrName').val(user.usrName);
			$('.usrIn #usrNick').val(user.nickname);
			$('.usrIn #usrPhone').val(user.phoneNum);
			$('#usrClsSetting #clsIcon').attr('name', user.usrIcon);
			$('#usrClsSetting #clsIcon').attr('src', '/godgamez/res/user/icon/' + user.usrIcon + '.jpg');
...
function fixUser() {
	...
	$.ajax({
		...
		if(user != null) {
			...
			if(user.usrPw != $('.usrIn #usrPw').val()) pw = $('.usrIn #usrPw').val();
			else {
				$('.usrIn #usrPw').val("");
				$('.usrIn #usrPwAgain').val("");
				$('#usrPwChk').empty();
				$('#usrPwAgainChk').empty();
				pw = user.usrPw;
			}
			if(user.nickname != $('.usrIn #usrNick').val()) nick = $('.usrIn #usrNick').val();
			else {
				$('.usrIn #usrNick').val("");
				$('#usrNickChk').empty();
				nick = user.nickname;
			}
			if(user.phoneNum != $('.usrIn #usrPhone').val()) tel = $('.usrIn #usrPhone').val();
			else {
				$('.usrIn #usrPhone').val("");
				$('#usrPhoneChk').empty();
				tel = user.phoneNum;
			}
			if(user.usrIcon != $('#usrClsSetting #clsIcon').attr('name')) icon = $('#usrClsSetting #clsIcon').attr('name');
			else icon = user.usrIcon;
			
			fixData = {usrCode: usrCode, position: posi, usrId: id, usrPw: pw, usrName: name, nickname: nick,
					birthday: bDay, phoneNum: tel, usrLv: lv, gold: gold, regDate: rDate, usrIcon: icon}
		}
	})
	
	if(fixData.usrId.length) {
		$.ajax({
...
function addClsFor(userCode) {
	/* 클래스 초기화 */
	$.ajax({
		url: "/godgamez/userclass/reset/" + userCode,
		type: "DELETE",
		success: function(result) {
        	if(usrclss.length) {
    			$.each(usrclss, (idx, usrcls) => {
	...
	<div class='container'>
		<div class='row my-5'>
			<div class='col'>
				...
				<div class='row ml-3 mt-4'>
					<small> 이름 변경은 직접 메일로 직접 문의주시길 바랍니다. (신분증 첨부 필수)<br> 정보를 변경하고자 하는 정보만 새로 입력하시기 바랍니다.</small>
				</div>
				<div class='row mt-3 mx-5 justify-content-center text-nowrap usrIn' onmousemove='chkInput(2)'>
					<div class='col-5'>
				...
				<div class='row mt-4 justify-content-center'>
					...
					<button type='button' class='btn btn-secondary col-5' onclick='fixUser()' id='nextStepBtn'>
						변경하기
					</button>
				</div>
				<hr>
	...
	<div class='modal-dialog'>
		<div class='modal-content'>
			...
			<div class='modal-footer justify-content-center'>
				...
				<a href='/godgamez/user/join/step1' class='btn btn-secondary'>&nbsp;&nbsp;&nbsp;예&nbsp;&nbsp;&nbsp;</a>
			</div>
		</div>
	</div>
</div>
