			...
			if(players.length) {
				...
				$.each(players, (idx, player) => {
					...
					playerList.push(
						`<tr id='usrDetail'>
							<td class='checkCol'>
								<input type='checkbox' name='check1' class='usrCode' value='\${player.usrCode}'>
							</td>
			...
			if(outs.length) {
				...
				$.each(outs, (idx, out) => {
					...
					outList.push(
						`<tr id='usrDetail'>
							<td class='checkCol'>
								<input type='checkbox' name='check2' class='usrCode' value='\${out.usrCode}'>
							</td>
			...
			if(noobs.length) {
				...
				$.each(noobs, (idx, noob) => {
					...
					noobList.push(
							`<tr id='usrDetail'>
								<td class='checkCol'>
									<input type='checkbox' name='check3' class='usrCode' value='\${noob.usrCode}'>
								</td>
			...
			if(gms.length) {
				...
				$.each(gms, (idx, gm) => {
					...
					gmList.push(
							`<tr id='usrDetail'>
								<td class='checkCol'>
									<input type='checkbox' name='check4' class='usrCode' value='\${gm.usrCode}'>
								</td>
					...
					$.each(users, (idx, user) => {
						...
						if(user.position != 'GM') {
							...
							userList.push(
								`<tr id='usrDetail'>
									<td class='checkCol'>
										<input type='checkbox' name='check1' class='usrCode' value='\${user.usrCode}'>
									</td>
					...
					$.each(users, (idx, user) => {
						...
						if(user.position == 'OUT') {
							...
							userList.push(
								`<tr id='usrDetail'>
									<td class='checkCol'>
										<input type='checkbox' name='check2' class='usrCode' value='\${user.usrCode}'>
									</td>
									<td id='code' onclick='getUser(\${user.usrCode}); getUserClasses(\${user.usrCode});'>\${user.usrCode}</td>
									<td id='usrPosition' onclick='getUser(\${user.usrCode}); getUserClasses(\${user.usrCode});'>\${user.position}</td>
									<td id='usrId' onclick='getUser(\${user.usrCode}); getUserClasses(\${user.usrCode});'>\${user.usrId}</td>
					...
					$.each(users, (idx, user) => {
						...
						if(user.position == 'NOOB') {
							...
							userList.push(
								`<tr id='usrDetail'>
									<td class='checkCol'>
										<input type='checkbox' name='check3' class='usrCode' value='\${user.usrCode}'>
									</td>
									<td id='code' onclick='getUser(\${user.usrCode}); getUserClasses(\${user.usrCode});'>\${user.usrCode}</td>
									<td id='usrPosition' onclick='getUser(\${user.usrCode}); getUserClasses(\${user.usrCode});'>\${user.position}</td>
									<td id='usrId' onclick='getUser(\${user.usrCode}); getUserClasses(\${user.usrCode});'>\${user.usrId}</td>
					...
					$.each(users, (idx, user) => {
						...
						if(user.position == 'GM') {
							...
							userList.push(
								`<tr id='usrDetail'>
									<td class='checkCol'>
										<input type='checkbox' name='check4' class='usrCode' value='\${user.usrCode}'>
									</td>
									<td id='code' onclick='getUser(\${user.usrCode}); getUserClasses(\${user.usrCode});'>\${user.usrCode}</td>
									<td id='usrPosition' onclick='getUser(\${user.usrCode}); getUserClasses(\${user.usrCode});'>\${user.position}</td>
									<td id='usrId' onclick='getUser(\${user.usrCode}); getUserClasses(\${user.usrCode});'>\${user.usrId}</td>
	...
	$.ajax({
		...
		success: function(usrclss) {
        	if(usrclss.length) {
    			...
    			$('#getUsrModal #addClsBtn').removeAttr("disabled");
       			$('#getUsrModal #rmvClsBtn').attr("disabled", true);
       			$('#getUsrModal #usrClssCnt').text("현재 0개");
...
function listCls(userCode) {
	$('.checkCol #checkall10:checked').prop('checked', false);
	...
	$.ajax({
		url: '/godgamez/user/class/list/' + userCode,
		method: 'post'
	}).done(clss => {
		if(clss.length) {
			let clsList = [];
	...
	if(keyword == null || keyword == '') {
		...
		$('#srchClsTBody #clsDetail').each(function() {
			...
			if(isSrch) {
				srchClsList.push(
					"<tr id='clsDetail'>" +
						"<td class='checkCol'><input type='checkbox' id='choosenClsId' name='check10' value=" + clsId + "></td>" +
...
function fixProc(userCode) {
	...
	$('#getUsrModal #newUsrId').val($('#getUsrModal input[id="usrId"]').val());
	$('#getUsrModal #newUsrPw').val($('#getUsrModal input[id="usrPw"]').val());
	$('#getUsrModal #newUsrName').val($('#getUsrModal input[id="usrName"]').val());
	$('#getUsrModal #newNickname').val($('#getUsrModal input[id="nickname"]').val());
	$('#getUsrModal #newBirthday').val($('#getUsrModal input[id="birthday"]').val());
	$('#getUsrModal #newPhoneNum').val($('#getUsrModal input[id="phoneNum"]').val());
...
function checkInput(j) {
	...
	if(j == "fix") {
		...
		if($('#addUsrModal #inPhoneNum').val().replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1').length) {
			...
			if(inPhoneNum.length < 10 || 11 < inPhoneNum.length)
				$('#addUsrModal #addPhoneNumChk').text("사용 불가] 10자 이상 11자 이하로 입력하세요.");
...
/* 인풋값 이상있는 지 없는 지 - 관리자페이지(회원) 회원 추가 시 */
function chkInput2(num) {
	$('#addUsrInBtn').attr('disabled', true);
	allInput = $('#addUsrModal small').length;
	goodInput = $('#addUsrModal .text-success').length;
	wrongInput = $('#addUsrModal .text-danger').length;
	if(allInput - goodInput == 1 && wrongInput < 1 && num <= allInput
			&& $('#positionOpt option:selected').val() != 'none') {
		$('#addUsrInBtn').removeAttr('disabled');
	}
}
...
$(document).ready(function() {
	...
	$('#listUsrsBtn').click(function() {
		...
		if(tabId == 'pUsrBtn') {
			getAllPlayers();
		} else if(tabId == 'oUsrBtn') {
			getOuts();
		} else if(tabId == 'nUsrBtn') {
			getNoobs();
		} else if(tabId == 'mUsrBtn') {
			getGms();
		}
	})
	...
	$('#usrSrchBtn').click(function() {
		...
		chkbxNm = $('.active').find('table').find('tbody').find('#usrDetail').find('td').children('input').attr('name');
		
		if(usrSrchDataVal.length < 1 || opt == "none") {
			modal("회원", "검색", "실패10", "조건을 선택하고 검색어를 입력하세요.")
		} else {
			switch(opt) {
	...
	$('.usrPlayerBtn').click(function() {
		let noobsCode = $('.usrCode:checked').val();
	...
	<%@ include file='../include/gnb.jsp' %>
		<div class='col' id='adminBody'>
			<div class='row mt-3 mx-3 justify-content-between'>
				...
				<small class='text-muted' id='usrSrchCnt'></small>
			</div>
			<hr>
			<div class='row mt-2'>
				...
				<div class='col-6 d-flex justify-content-end text-nowrap' id=usrSrchDiv>
					...
					<input type='text' class='form-control' placeholder='검색어를 입력하세요.' id='usrSrchIn'>
					...
					<div class='tab-content'>
						<div class='tab-pane fade show active' id='pUsrBoard'>
							...
							<div class='row text-nowrap'>
								<div class='col-5'>
									<div class='btn-group'>
										<button class='btn btn-outline-secondary usrUnregBtn'>삭제</button>
										<button class='btn btn-outline-secondary usrPlayerBtn'>인증</button>
										<button class='btn btn-outline-secondary cnclOutBtn'>복구</button>
									</div>
								</div>
								...
								<div class='col-5'>
									<div class='btn-group float-right'>
										<button class='btn btn-secondary' onclick='resetAddUsrModal()'
											data-toggle='modal' data-target='#addUsrModal'>추가</button>
									</div>
								</div>
							</div>
						</div>
...
<!-- 모달 -->
<div id='addUsrModal' class='modal fade modal-fullsize modal-center' onmousemove="chkInput2(5)" tabindex='-1'>
				...
				<div class='row'>
					<div class='col'>
						<table class='border-0 small text-nowrap w-100'>
							<tr>
								...
								<td width='25%' id='usrId'>
									<input type='email' class='form-control' id='inUsrId'
										onblur="checkInput('add')" onchange="chkInput2(5)">
								</td>
								<td width='5%'></td>
								<td width='10%'>
								...
								<td width='50%' id='usrPw'>
									<input type='text' class='form-control' id='inUsrPw'
										onchange="chkInput2(5)" onblur="checkInput('add')">
								</td>
							</tr>
							<tr>
							...
							<tr>
								...
								<td width='25%' id='usrName'>
									<input type='text' class='form-control' id='inUsrName'
										onchange="chkInput2(5)" onblur='checkInput("add")'>
								</td>
								<td width='5%'></td>
								<td width='10%'>
								...
								<td width='50%' id='nickname'>
									<input type='text' class='form-control' id='inNickname'
										onchange="chkInput2(5)" onblur='checkInput("add")'>
								</td>
							</tr>
							<tr>
							...
							<tr>
								...
								<td width='50%' id='phoneNum'>
									<input type='tel' class='form-control' id='inPhoneNum'
										onchange="chkInput2(5)" onblur="checkInput('add')">
								</td>
							</tr>
							<tr>
						...
						<div class='row justify-content-end mt-3 mr-1' id='addUsrModalBtn'>
							<button type='button' class='btn btn-outline-secondary' id='addUsrInBtn'
	...
	<div class='modal-dialog'>
		<div class='modal-content'>
			<div class='modal-header'>
				<h6>회원 조회 및 수정</h6>
				<div class='justify-content-end'>
					<small class='usrCode' class='mr-2'></small>
		...
		<div class='modal-content'>
			...
			<div class='modal-body text-center'>
				<div class='row'>
					...
					<input type='text' class='form-control ml-3 w-50' id='srchClsIn' placeholder='검색어를 입력하세요'>
