<%@ page language='java' contentType='text/html; charset=utf-8' pageEncoding='utf-8'%>
<header class='container-fluid' id='adminHeader'>
	<div class='row'>
		<div class='text-center'>
			<a class='btn text-light' href='/godgamez/admin'>
				<img src='/godgamez/res/logo/logo_dark.jpg' id='logoImg' alt='어두운로고이미지'>
			</a>
		</div>
		<div class='col pt-4 pb-1'>
			<small class='text-left font-weight-bold'>ADMINISTRATOR</small>
		</div>
		<div class='col pt-4 pb-1 d-flex justify-content-end'> 
			<a href="/godgamez/user/logout" id="logOutLink" class='mr-3'>
				<small class='text-light'>Log out</small>
			</a> 
			<a href='/godgamez/' class='mr-3'> 
				<small class='text-light'>사이트로</small>
			</a>
		</div>
	</div>
</header><!-- <c:url value="/res/logo_dark.png"/> -->