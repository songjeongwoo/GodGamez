	...
	$.ajax({
		...
		if(clss.length) {
			...
			$.each(clss, (idx, cls) => {
				//클래스 - 사용퀘스트
				$.ajax({
					url: '/godgamez/class/getInUseCls',
					async: false,
				...
				clsList.push(
					`<tr class='clsDetail'>
		  				...
		  				<td class='clsId'>\${cls.clsId}</td>
		  				<td class='mainCtg'>\${cls.mainCtg}</td>
		  				<td class='subCtg'>\${cls.subCtg}</td>
		  				<td class='clsName'>\${cls.clsName}</td>
...
function delB() {
	...
	if(useQstCnt == 0) {
		$.ajax({
			url: `/godgamez/class/del/\${$('#clsChk:checked').closest('.clsDetail').children('.clsId').text()}`,
			method: 'delete'
		}).done(result => {
			if(result) {
				modal('클래스', '삭제', '성공');
...
$(document).ready(function() {
	...
	$.ajax({
	    ...
	    success: function(result) {
			if(result == 2) {
				getClss('/godgamez/class/list')
				$('#refreshBtn').click(() => {
					let navTab = $('#category').find('.active').text()
					if(navTab == '전체') getClss("/godgamez/class/list")
					else if(navTab == '공부') getClss("/godgamez/class/list/study")
					else if(navTab == '운동') getClss("/godgamez/class/list/exercise")
						...
						<div class='col-5 mb-0'>
							<nav class='nav nav-tabs' id='category'>
								<a class='nav-link btn-outline-secondary active' data-toggle='tab' href='#allClsBoard'
									id='allTab' onclick='getClss("/godgamez/class/list")'>전체</a>
								<a class='nav-link btn-outline-secondary' data-toggle='tab' href='#allClsBoard'
									id='stdTab' onclick='getClss("/godgamez/class/list/study")'>공부</a>
								<a class='nav-link btn-outline-secondary' data-toggle='tab' href='#allClsBoard'
									id='excTab' onclick='getClss("/godgamez/class/list/exercise")'>운동</a>
							</nav>
						</div>
								...
								<div class='tab-pane fade show active' id='allClsBoard'>
									<div style="max-height:20rem; overflow-y:auto; overflow-x: hidden;">
										<table class='table table-sm table-secondary border mb-0 table-hover text-center'>
											<thead>
												<tr>
													<th class='checkCol'><input type='checkbox' id='checkall1'/></th>
										   			<th class='clsId'>클래스ID</th>
										     		<th class='mainCtg'>대분류</th>
										     		<th class='subCtg'>중분류</th>
										     		<th class='clsName'>이름</th>
										     		<th class='useQstCnt'>사용퀘스트</th>
											    </tr>
											</thead>
								...
								<tr>
									...
									<td>
										<input type='text' placeholder='2글자 이상의 한글로 입력하세요.'
											class='text-center form-control w-75 ml-3 subCtg'/>
									</td>
								</tr>
								<tr>
									<th>클래스명</th>
									<td>
										<input type='text' placeholder='2글자 이상의 한글로 입력하세요.'
											class='text-center form-control w-75 ml-3 clsName'/>
									</td>
								</tr>
							</tbody>
						</table>
